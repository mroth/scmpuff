...
func TestProcess(t *testing.T) {
	const (
		// localTestdata contains v2-only test fixtures specific to this parser.
	...
	testCases := []struct {
	...
	}{
		{
			...
			sampleFile: "process-untracked.porcelain-v2z.bin",
			want: &gitstatus.StatusInfo{
		...
		{
			testdata:   localTestdata,
			sampleFile: "process-changes.porcelain-v2z.bin",
			want: &gitstatus.StatusInfo{
				Branch: gitstatus.BranchInfo{
					Name:          "main",
					CommitsAhead:  0,
		...
		},
		{
			...
			// a [.A] status code in v2 that maps to ChangeUnstagedNewFile.
			// Fixture from user-submitted debug dump (jujutsu colocated repo).
			testdata:   sharedTestdata,
			sampleFile: "issue86.porcelain-v2z.bin",
			want: &gitstatus.StatusInfo{
				Branch: gitstatus.BranchInfo{
					Name:          "main",
					CommitsAhead:  0,
