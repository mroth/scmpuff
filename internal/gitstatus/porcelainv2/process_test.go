...
func Test_decodeXY(t *testing.T) {
	testCases := []struct {
		...
		{
			// UpdatedUnmerged should never appear in a ChangedEntry's X
			xy:      statusv2.XYFlag{X: statusv2.UpdatedUnmerged, Y: statusv2.Modified}, // [UM]
			wantErr: true,
		},
		{
			// UpdatedUnmerged should never appear in a ChangedEntry's Y
			xy:      statusv2.XYFlag{X: statusv2.Modified, Y: statusv2.UpdatedUnmerged}, // [MU]
			wantErr: true,
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.xy.String(), func(t *testing.T) {
			...
			if (err != nil) != tc.wantErr {
				t.Errorf("decodeXY(%s): wantErr=%v, got err=%v", tc.xy, tc.wantErr, err)
			}
...
func Test_decodeUnmergedXY(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.xy.String(), func(t *testing.T) {
			...
			if (err != nil) != tc.wantErr {
				t.Errorf("decodeUnmergedXY(%s): wantErr=%v, got err=%v", tc.xy, tc.wantErr, err)
			}
			if got != tc.want {
...
func Test_extractBranch(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if (err != nil) != tc.wantErr {
				t.Errorf("extractBranch: wantErr=%v, got err=%v", tc.wantErr, err)
			}
			if got != tc.want {
				t.Errorf("extractBranch: want %+v, got %+v", tc.want, got)
			}
		})
	}
}
